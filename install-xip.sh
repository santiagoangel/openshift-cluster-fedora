...
git clone -b 1.4.1 https://github.com/santiagoangel/openshift-cluster-fedora

#apply hosts

rm -Rf /etc/hosts
cp openshift-cluster-fedora/hosts-xip /etc/hosts

#ssh keys
ssh-keygen -t rsa
ssh-copy-id root@cloud.successfulsoftware.192.168.88.98.xip.io

#ansible run

ansible-playbook -i ./openshift-cluster-fedora/inventory-registry-all-xip.erb ./openshift-ansible/playbooks/byo/config.yml -vvv

#then set a new user: e.g. developer
htpasswd /etc/origin/master/htpasswd developer

#open https://cloud.successfulsoftware.192.168.88.98.xip.io:8443/console/
